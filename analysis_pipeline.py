...
def pipe(pressures,
    ...
    # sttc params
    dt_maxs = np.array([.015, .032])
    dt_mins = np.array([0, 0])
    ...
    for pressure, file_path in zip(pressures, files):
        ...
        mea_data.compute_sttc(dt_max=dt_maxs, dt_min=dt_mins, use_numba=False)
        print()

        print(f'Calculating STTC to random comparison with dt_max = {dt_maxs} and dt_min = {dt_mins}......')
        mea_data.compare_STTC_to_random(bins_data='auto', bins_sim='auto', plot=False, method='all', use_numba=False, n_splits=5)
        print()
...
if __name__ == '__main__':
    base_paths = ['C:\\Users\\bow-lab\\Documents\\Code\\data\\ABAB_4\\241126\\25510\\']
